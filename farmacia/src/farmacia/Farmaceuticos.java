...
public class Farmaceuticos extends javax.swing.JInternalFrame {
    ...
    public Farmaceuticos() {
        initComponents();
        this.setTitle("Farmacéuticos");
        bloquear();
        cargarTabla("");
        
        tblFarmaceutico.setAutoResizeMode(tblFarmaceutico.AUTO_RESIZE_OFF);
    ...
    public void modificar() {
        ...
        txtCedula.setText(tblFarmaceutico.getValueAt(fila, 0).toString().trim());
        txtNombre1.setText(tblFarmaceutico.getValueAt(fila, 1).toString().trim());
        txtNombre2.setText(tblFarmaceutico.getValueAt(fila, 2).toString().trim());
        txtApellido1.setText(tblFarmaceutico.getValueAt(fila, 3).toString().trim());
        txtApellido2.setText(tblFarmaceutico.getValueAt(fila, 4).toString().trim());
        txtTelefono.setText(tblFarmaceutico.getValueAt(fila, 5).toString().trim());
        txtDirreccion.setText(tblFarmaceutico.getValueAt(fila, 6).toString().trim());
    ...
    public void cargarTabla(String dato) {
     ...
     private void guardar() {
        ...
        if (txtCedula.getText().isEmpty()) {
            ...
            Connection cn = cc.conectar();
            String sql = "";
            String CED_FAR, NOM1_FAR, NOM2_FAR, APE1_FAR, APE2_FAR, TEL_FAR, DIR_FAR,ADMIN;
            CED_FAR = txtCedula.getText();

            if (CED_FAR.equals(tblFarmaceutico)) {
                JOptionPane.showMessageDialog(null, "Cédula ya existe!");
            }
            NOM1_FAR = txtNombre1.getText();
            NOM2_FAR = txtNombre2.getText();
            APE1_FAR = txtApellido1.getText();
            APE2_FAR = txtApellido2.getText();
            DIR_FAR = txtDirreccion.getText();
            TEL_FAR = txtTelefono.getText();
          
            sql = "insert into farmaceutico(CED_FAR,NOM1_FAR,NOM2_FAR,APE1_FAR,APE2_FAR,TEL_FAR,DIR_FAR) values(?,?,?,?,?,?,?)"; // Las incógnitas representas los valores que voy a mandar en cada campo
            ...
            try {
                ...
                psd.setString(1, CED_FAR);
                psd.setString(2, NOM1_FAR);
                psd.setString(3, NOM2_FAR);
                psd.setString(4, APE1_FAR);
                psd.setString(5, APE2_FAR);
        ...
        Connection cn = cc.conectar();
        String sql;
        sql = "select COUNT(*) AS contar from farmaceutico where CED_FAR='" + txtCedula.getText() + "'";
        try {
            Statement psd = cn.createStatement();
            ResultSet rs = psd.executeQuery(sql);
            while (rs.next()) {
...
//            JOptionPane.showMessageDialog(null, ex);
        ...
        if (txtNombre1.getText().isEmpty()) {
            ...
            Connection cn = cc.conectar();
            String sql = "";
            sql = "UPDATE farmaceutico "
                    + "set NOM1_FAR='" + txtNombre1.getText() + "',"
                    + "NOM2_FAR='" + txtNombre2.getText() + "',"
                    + "APE1_FAR='" + txtApellido1.getText() + "',"
                    + "APE2_FAR='" + txtApellido2.getText() + "',"
                    + "TEL_FAR='" + txtTelefono.getText() + "',"
                    + "DIR_FAR='" + txtDirreccion.getText() + "'"
                    + "WHERE CED_FAR='" + txtCedula.getText() + "'";
        ...
        if (JOptionPane.showConfirmDialog(this, "Està seguro de eliminar el registro?") == 0) {
            conexion cc = new conexion();
            Connection cn = cc.conectar();
            String sql = "DELETE FROM farmaceutico WHERE CED_FAR='" + txtCedula.getText() + "'";
    ...
    private void initComponents() {
        ...
        jLabel3.setText("Datos del Farmacéutico:");
