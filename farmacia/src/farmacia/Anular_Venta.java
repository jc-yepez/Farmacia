 ...
 * @author Personal
 */
public class Anular_Venta extends javax.swing.JInternalFrame {

    /**
     * Creates new form Anular_Venta
     */
    public Anular_Venta() {
        initComponents();
        this.setTitle("Anular Venta");
        cargarTabla("");
        tblAnular.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent lse) {
                if (tblAnular.getSelectedRow() != -1) {//condicion para verificar si algo esta seleccionado
                    ...
                    ((JTextField) jdcFecha.getDateEditor().getUiComponent()).setText(tblAnular.getValueAt(fila, 1).toString().trim());

                    txtCedulaFar.setText(tblAnular.getValueAt(fila, 2).toString().trim());
                    txtCedulaCli.setText(tblAnular.getValueAt(fila, 3).toString().trim());

                    txtTotal.setText(tblAnular.getValueAt(fila, 4).toString().trim());
                     
                     String[] titulos = {"Nº Factura", "Código", "Cantidad", "Subtotal"};
                    ...
                    sql = "select * from detalle_factura where NUM_FAC_P='" + numFac + "'";
                    try {
                        Statement psd = cn.createStatement();
                        ResultSet rs = psd.executeQuery(sql);
                        while (rs.next()) {
                            registros[0] = rs.getString("NUM_FAC_P");
                            registros[1] = rs.getString("COD_MED_P");
                            registros[2] = rs.getString("CANT_FAC");
 ...
 DefaultTableModel modelo3;
  DefaultTableModel modelo4;
    public void cargarTabla(String dato) {
        String[] titulos = {"Nº Factura", "Fecha", "CI. Farmacéutico", "CI. Cliente", "Total"};
...
//                  + "where facturas_uno.NUM_FAC LIKE'%" + dato + "%'order by facturas_uno.NUM_FAC";
            sql = "select * from facturas_uno where NUM_FAC LIKE'%" + dato + "%'order by NUM_FAC";
        ...
        try {
            ...
            ResultSet rs = psd.executeQuery(sql);
            while (rs.next()) {
                registros[0] = rs.getString("NUM_FAC");
                registros[1] = rs.getString("FEC_FAC");
                registros[2] = rs.getString("CED_FAC_FARM");
                registros[3] = rs.getString("CED_FAC_CLI");
                registros[4] = rs.getString("TOTAL_FAC");
      ...
      public void eliminar() {
        if (JOptionPane.showConfirmDialog(this, "Està seguro de eliminar el registro?") == 0) {
            conexion cc = new conexion();
            Connection cn = cc.conectar();
            String sql = "DELETE FROM facturas_uno WHERE NUM_FAC='" + txtNumVenta.getText() + "'";
                        ...
                        .addGroup(jPanel7Layout.createSequentialGroup()
                            ...
                            .addGroup(jPanel7Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                ...
                                .addGroup(jPanel7Layout.createSequentialGroup()
                                    ...
                                    .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                                    .addComponent(txtCedulaFar, javax.swing.GroupLayout.PREFERRED_SIZE, 174, javax.swing.GroupLayout.PREFERRED_SIZE)))
                            .addGap(12, 12, 12))))
                .addContainerGap(18, Short.MAX_VALUE))
        );
...
//        cargarTabla(txtBuscar.getText());
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Anular_Venta.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                new Anular_Venta().setVisible(true);
            }
        });
    }
